...
```{r}
#Recall for training set we have: 
...
dat_test_sift <- read.csv("../data/test/sift_features.csv")
dat_test_sift <- t(as.matrix(dat_test_sift))
#Import the set of new visual features (constructed outside of R)
dat_test_new <- read.csv("../data/test/new_features.csv")  #or other file name
dat_test_new <- t(as.matrix(dat_test_new))

```

### Construct the new visual features   
